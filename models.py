...
class BaseModel(object):
    ...
    def __init__(self, vocab_model, batch_size, embed_size, noise_sample_size = None, noise_dist = None, reg = 0.0, optimize ='rms'):
...
class CBOW(BaseModel):
    ...
    def make_network(self,):
        ...
        w_in_x_sum = W_in[X,:].mean(axis = 1) #T.sum(w_in_x, axis = 1) #(batch_size, hidden_state) # selects the rows in W_in, per row in X, then sums those rows
        ...
        #self.get_l2_norms = theano.function(inputs = [], outputs = l2_norms)
        #self.get_numerator = theano.function(inputs = [], outputs = numerator)
        demoninator = l2_norms[:, np.newaxis].dot(l2_norms[np.newaxis, :])
        cosine_sim = numerator / demoninator
        self.cosine_similarity = theano.function(inputs = [], outputs = cosine_sim) 
        self.cosine_similarity_cell = theano.function(inputs = [V1, V2], outputs = cosine_sim[V1, V2]) 
        self.cosine_similarity_row = theano.function(inputs = [V1], outputs = cosine_sim[V1, :]) 
