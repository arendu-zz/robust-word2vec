...
if __name__ == '__main__':
    ...
    if options.model == 'cbow':
        ...
        model = SkipGram(vocab_model = vocab, batch_size = options.batch_size, embed_size = options.embed_size, 
                noise_sample_size= options.noise_sample_size, noise_dist = noise_dist,
                reg=0.0, optimize = options.optimizer, shared_X = X_full, shared_Y = Y_full)

        model_input = SkipGram(vocab_model = vocab, batch_size = options.batch_size, embed_size = options.embed_size, 
                noise_sample_size= options.noise_sample_size, noise_dist = noise_dist,
                reg=0.0, optimize = options.optimizer)

    print 'starting shared loss', datetime.now().strftime('%Y-%m-%d %H:%M:%S')
    sys.stdout.flush()
    for e_idx in xrange(options.epochs):
        l = model.__loss__()
        print l, datetime.now().strftime('%Y-%m-%d %H:%M:%S')

    print 'starting batch loss w inputs', datetime.now().strftime('%Y-%m-%d %H:%M:%S')
    for e_idx in xrange(options.epochs):
        l = model_input.loss(options.batch_size, X_full, Y_full)
        print l, datetime.now().strftime('%Y-%m-%d %H:%M:%S')
